...
def stft(x, L, hop, transform=np.fft.fft, win=None, zp_back=0, zp_front=0):
  ...
  y = np.concatenate((np.zeros((y.shape[0], zp_front)), y, np.zeros((y.shape[0], zp_back))), axis=1)
...
### FreqVec: given FFT size and sampling rate, returns a vector of real frequencies
def freqvec(N, Fs, centered=False):
  ...
  # Shift positive/negative frequencies if needed. Again (1-N%2) for even/odd length
  if centered:
    return vec
  else:
